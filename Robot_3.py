...
class Robot:
    ...
    def __init__(self):
        ...
        # self.pickedup_thread = threading.Thread(target=self.picked_up)
        # self.pickedup_thread.start()
    ...
    def spintonextline(self,desired_heading):
        ...
        for i in range(abs(dir)):
            ...
            while turning:
                ...
                if dir > 0:
                    # self.motor.set(-380*SLOPE, 380*SLOPE)
                    self.motor.set(-405*SLOPE, 405*SLOPE)
                    time.sleep(0.002)
    ...
    #         # if ir_left_state == 0 and ir_center_state == 0 and ir_right_state == 0:
    #         #     self.driving_pause = True
    #         while ir_left_state == 0 and ir_center_state == 0 and ir_right_state == 0:
    #             (ir_left_state, ir_center_state, ir_right_state) = self.infrared.readSensors()
    #             current_time = time.time()
    #             if not self.left_us.object_present and not self.right_us.object_present:
    #                 if current_time - start_time > 5:
    #                     self.motor.setlinear(0)
    #                     self.driving_pause = True
    ...
    def driving_loop(self):
        ...
        while not self.driving_stopflag:
            ...
            if self.goal_stopflag:
                ...
                global long
                global lat
                self.find_goal(self.intersection(long, lat), self.intersection(goal_long, goal_lat))
    ...
    def heading_to_target(self, goal):
        ...
        print("Entered heading to target. Current intersection heading: ", current_intersection.headingToTarget)
        ...
        if self.intersection(goal_long, goal_lat) != None:
            ...
            while temp_target.lat != start.lat or temp_target.long != start.long:
                for i in range(0,len(temp_target.streets)):
                    ...
                    if (temp_target.streets[i] == UNEXPLORED or temp_target.streets[i] == CONNECTED) and checking_intersection.headingToTarget == None:
                        print(temp_target.streets[i])
    ...
    def find_goal(self, start, goal):
        ...
        global long
        global lat
        print("goal_long: ", goal_long)
        print("goal_lat: ", goal_lat)
        if goal_long == long and goal_lat == lat:
            print("Reached Target!")
            self.driving_pause = True
            self.goal_stopflag = False
            closest_intersection = self.intersection(long, lat)
            closest_intersection.headingToTarget = None
        else:
            if self.intersection(goal_long, goal_lat) != None:
                self.djikstra(self.intersection(long, lat), goal)
            else:
                    ...
                    for i in intersections:
                        ...
                        # if i.streets.index('UNEXPLORED') != -1:
                            ...
                            if dist < shortest_dist:
                                ...
                                closest_intersection = i
                    print("Closest intersection is: ", closest_intersection)
                    print("Current Intersection is, Djikstras: ", self.intersection(long, lat))
                    self.djikstra(self.intersection(long, lat), closest_intersection)
                    for i in range(0,len(closest_intersection.streets)):
                        ...
                        if closest_intersection.streets[i] == UNEXPLORED:
                            if i == 0:
                                dist2 = abs(closest_intersection.long - goal_long) + abs(closest_intersection.lat + 1 - goal_lat)
                                print("d1 North ", dist)
                            elif i == 1:
                                dist2 = abs(closest_intersection.long - 1 - goal_long) + abs(closest_intersection.lat - goal_lat)
                                print("d2 West ", dist)
                            elif i == 2:
                                dist2 = abs(closest_intersection.long - goal_long) + abs(closest_intersection.lat - 1 - goal_lat)
                                print("d3 South ", dist)
                            ...
                            if dist2 < shortest_dist2:
                                ...
                                desired_heading = i
                    print("1) left the for loop")
                    print(self.intersection(long, lat))
                    closest_intersection.headingToTarget = desired_heading
                    print("Heading to target of the closest intersection: ", closest_intersection.headingToTarget)
                else:
                    print("No path!")
                    self.driving_pause = True
                    self.goal_stopflag = False
